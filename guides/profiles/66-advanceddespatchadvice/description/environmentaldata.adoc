...
|EPD Product name |EPDProductName |BEAST:4.0 |The product name relating to an EPD
|EPD HR URL |EPDHRURL |BEAST:4.0 |URL to the human readable version of an EPD, for verification purposes
|EPD Product Name Data Set |EPDProductNameDataSet |BEAST:4.0 |The product name relating to a individual dataset in the EPD
...
The first two (EPD Dataset ID and URL) are used when the EPD is machine readable.  
The next four (Programme operator, EPD name, EPD nr and HR URL) are used when the EPD is not machine readable 
The last (EPD Product name dataset) is needed when the non-machine readable EPD has more than one data set   
If the EPD is only accessible as a PDF see <<reference-to-other-documents>> for more info how to link or attach the PDF.
