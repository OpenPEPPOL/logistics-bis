...
<pattern xmlns="http://purl.oclc.org/dsdl/schematron">
    ...
    <rule context="cbc:ProfileID">
        <assert id="PEPPOL-T19-R017"
            ...
            flag="fatal">[PEPPOL-T19-R017]-An order transaction SHALL use profile catalogue only or catalogue without response.</assert>
    </rule>

	<rule context="cbc:CustomizationID">
			<assert id="PEPPOL-T19-R018" 
					test="starts-with(normalize-space(.), 'urn:fdc:peppol.eu:poacc:trns:catalogue:3')"
					flag="fatal">[PEPPOL-T19-R018]-Specification identifier SHALL start with the value 'urn:fdc:peppol.eu:poacc:trns:catalogue:3'.</assert>
	</rule>    
    
    <rule context="/ubl:Catalogue/cac:ValidityPeriod">
        <assert id="PEPPOL-T19-R001"
                test="$CatalogueValidityEnd &gt;= $CatalogueValidityStart"
                flag="fatal">[PEPPOL-T19-R001]-A validity period end date SHALL be later or equal to a validity period start date</assert>
    </rule>
    ...
    <rule context="cac:SellerSupplierParty">
        <assert id="PEPPOL-T19-R004"
                ...
                flag="fatal">[PEPPOL-T19-R004]-A catalogue supplier SHALL contain the full name or an identifier</assert>
    </rule>

    <rule context="cac:ContractorCustomerParty">
        <assert id="PEPPOL-T19-R005"
                test="(cac:Party/cac:PartyName/cbc:Name) or (cac:Party/cac:PartyIdentification/cbc:ID)"
                flag="fatal">[PEPPOL-T19-R005]-A catalogue customer SHALL contain the full name or an identifier</assert>
    </rule>
    ...
    <rule context="cac:CatalogueLine">
        ...
        <assert id="PEPPOL-T19-R008"
                ...
                flag="fatal">[PEPPOL-T19-R008]-Maximum quantity SHALL be greater than zero</assert>

        <assert id="PEPPOL-T19-R009"
                test="not(cbc:MinimumOrderQuantity) or number(cbc:MinimumOrderQuantity) &gt;= 0"
                flag="fatal">[PEPPOL-T19-R009]-Minimum quantity SHALL be greater than zero</assert>
        ...
        <assert id="PEPPOL-T19-R010"
                ...
                flag="fatal">[PEPPOL-T19-R010]-Maximum quantity SHALL be greater or equal to the Minimum quantity</assert>
        ...
        <assert id="PEPPOL-T19-R007"
            ...
            flag="fatal">[PEPPOL-T19-R007]-Catalogue line validity period SHALL be within the range of the whole catalogue validity period</assert>
        <assert id="PEPPOL-T19-R013"
            test="($CatalogueLineValidityStart &lt;= $CatalogueLineValidityEnd)"
            flag="fatal">[PEPPOL-T19-R013]-A line validity period end date SHALL be later or equal to the line validity period start date
        </assert>
    </rule>
    ...
    <rule context="cac:RequiredItemLocationQuantity">
        ...
        <assert id="PEPPOL-T19-R006"
                ...
                flag="fatal">[PEPPOL-T19-R006]-Prices of items SHALL not be negative</assert>
        ...
        <assert id="PEPPOL-T19-R011"
            ...
            flag="fatal">[PEPPOL-T19-R011]-Price validity start date SHALL be within the range of the catalogue line or catalogue validity period</assert>
        <assert id="PEPPOL-T19-R016"
            test="($CataloguePriceValidityStart &lt;= $CataloguePriceValidityEnd)"
            flag="fatal">[PEPPOL-T19-R016]-A price validity period end date SHALL be later or equal to the price validity period start date
        </assert>
    </rule>
    ...
    <rule context="cac:ClassifiedTaxCategory">
        <assert id="PEPPOL-T19-R014"
            ...
            flag="fatal">[PEPPOL-T19-R014]-Each Tax Category SHALL have a TAX category rate, except if the catalogue line is not subject to TAX.</assert>
        <assert id="PEPPOL-T19-R015"
            test="not(normalize-space(cbc:ID)='S') or (cbc:Percent) &gt; 0"
            flag="fatal">[PEPPOL-T19-R015]-When TAX category code is "Standard rated" (S) the TAX rate SHALL be greater than zero.</assert>
    </rule>

    <rule context="cac:Item">
        <assert id="PEPPOL-T19-R012"
                test="(cac:StandardItemIdentification/cbc:ID) or (cac:SellersItemIdentification/cbc:ID)"
                flag="fatal">[PEPPOL-T19-R012]-Each item in a Catalogue line SHALL be identifiable by either "item sellers identifier" or "item standard identifier"</assert>
    </rule>
