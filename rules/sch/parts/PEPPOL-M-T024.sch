...
<let name="syntaxError" value="string('[PEPPOL-T024-S003] A Call For Tenders document SHOULD only contain elements and attributes described in the syntax mapping. - ')"/> 
    ...
    <rule context="ubl:CallForTenders/cbc:CustomizationID">
        <assert id="PEPPOL-T024-R002" flag="fatal" test="normalize-space(.) = 'urn:fdc:peppol.eu:prac:trns:t024:1:0'">[PEPPOL-T024-R002] CustomizationID value MUST be 'urn:fdc:peppol.eu:prac:trns:t024:1:0'</assert>
    ...
    <rule context="ubl:CallForTenders/cbc:ProfileID">
        <assert id="PEPPOL-T024-R003" flag="fatal" test="normalize-space(.) = 'urn:fdc:peppol.eu:prac:bis:p013:1:0'">[PEPPOL-T024-R003] ProfileID value MUST be 'urn:fdc:peppol.eu:prac:bis:p013:1:0'</assert>
    ...
    <rule context="ubl:CallForTenders/cac:TenderingTerms/cbc:AdditionalConditions">
        ...
        <assert id="PEPPOL-T024-S398" flag="warning" test="not(./@*)"><value-of select="$syntaxError"/>[PEPPOL-T024-S398] AdditionalConditions SHOULD NOT contain any attributes.</assert>
    </rule>
    ...
    <rule context="ubl:CallForTenders/cac:TenderingProcess/cbc:ProcedureCode">
        ...
        <assert id="PEPPOL-T024-R042" flag="fatal" test="(normalize-space(.) = '1' and not(exists(../cac:ParticipationRequestReceptionPeriod))) or (normalize-space(.) != '1')">[PEPPOL-T024-R042] Participation Request Reception Period MUST not be given for proceduretypes without participation contest.</assert>
    </rule>
    ...
    <rule context="ubl:CallForTenders/cac:TenderingProcess/cbc:ContractingSystemCode">
        ...
        <assert id="PEPPOL-T024-R039" flag="fatal" test="normalize-space(.) = ('1', '2', '3')">[PEPPOL-T024-R039] A Call For Tender is only allowed to use one of the following ContractSystemCodes: 'Public Contract', 'Establishment of a Framework agreement' or 'Setting up a Dynamic Purchasing System'.</assert>
    </rule>
