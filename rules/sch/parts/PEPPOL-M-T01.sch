...
<pattern xmlns="http://purl.oclc.org/dsdl/schematron">
    ...
    <let name="TAXamount" value="if(/ubl:Order/cac:TaxTotal/cbc:TaxAmount) then xs:decimal(/ubl:Order/cac:TaxTotal/cbc:TaxAmount) else 0"/>
        ...
        <rule context="cbc:ProfileID">
                <assert id="PEPPOL-T01-R031"
                        ...
                        flag="fatal">[PEPPOL-T01-R031]-An order transaction SHALL use profile order only, ordering or advanced_ordering.</assert>
        </rule>
 
		<rule context="cbc:CustomizationID">
				<assert id="PEPPOL-T01-R034" 
						test="starts-with(normalize-space(.), 'urn:fdc:peppol.eu:poacc:trns:order:3')"
						flag="fatal">[PEPPOL-T01-R034]-Specification identifier SHALL start with the value 'urn:fdc:peppol.eu:poacc:trns:order:3'.</assert>
		</rule>
    ...
    <rule context="cbc:Amount | cbc:TaxAmount | cbc:LineExtensionAmount | cbc:PriceAmount | cbc:BaseAmount | cac:AnticipatedMonetaryTotal/cbc:*">
        <assert id="PEPPOL-T01-R003"
                ...
                flag="fatal">[PEPPOL-T01-R003]-An order SHALL be stated in a single currency</assert>
    ...
    <rule context="cac:OriginatorCustomerParty">
        <assert id="PEPPOL-T01-R014"
                ...
                flag="fatal">[PEPPOL-T01-R014]-An order SHALL have the originator party name or an identifier</assert>
    </rule>
    ...
    <rule context="cac:PartyTaxScheme[cac:TaxScheme/cbc:ID='VAT']">
        <assert id="PEPPOL-T01-R026"
                ...
                flag="fatal">[PEPPOL-T01-R026]-When TAX is VAT then Party VAT identifiers SHALL have a prefix in accordance with ISO code ISO 3166-1 alpha-2 by which the country of issue may be identified. Nevertheless, Greece may use the prefix ‘EL’.</assert>
    </rule>
    ...
    <rule context="cac:AnticipatedMonetaryTotal">
        ...
        <assert id="PEPPOL-T01-R006"
                ...
                flag="fatal">[PEPPOL-T01-R006]-Expected total amount for payment SHALL NOT be negative</assert>
        <assert id="PEPPOL-T01-R007"
                test="$lineEtensionAmount &gt;=0"
                flag="fatal">[PEPPOL-T01-R007]-Expected total sum of line amounts SHALL NOT be negative</assert>
        <assert id="PEPPOL-T01-R008"
                test="$lineEtensionAmount = $sumLineExtensionAmount"
                flag="fatal">[PEPPOL-T01-R008]-Expected total sum of line amounts SHALL equal the sum of the order line amounts at order line level</assert>
        <assert id="PEPPOL-T01-R009"
                test="$allowanceTotalAmount = $sumAllowance"
                flag="fatal">[PEPPOL-T01-R009]-Expected total sum of allowance at document level SHALL be equal to the sum of allowance amounts at document level</assert>
        <assert id="PEPPOL-T01-R010"
                test="$chargeTotalAmount= $sumCharge"
                flag="fatal">[PEPPOL-T01-R010]-Expected total sum of charges at document level SHALL be equal to the sum of charge amounts at document level</assert>

        <assert id="PEPPOL-T01-R011"
                test="$taxexclusiveAmount = $lineEtensionAmount - $allowanceTotalAmount + $chargeTotalAmount"
                flag="fatal">[PEPPOL-T01-R011]-Expected total amount without TAX = Expected total sum of line amounts - Sum of allowances on document level + Sum of charges on document level</assert>
        <assert id="PEPPOL-T01-R016"
                ...
                flag="fatal">[PEPPOL-T01-R016]-Amount due for payment = Invoice total amount with TAX - Paid amount + Rounding amount.</assert>
        <assert id="PEPPOL-T01-R017"
                ...
                flag="fatal">[PEPPOL-T01-R017]-Expected total amount with TAX = Expected total amount without TAX + Order total TAX amount.</assert>
    </rule>
    ...
    <rule context="cac:AllowanceCharge[cbc:MultiplierFactorNumeric and not(cbc:BaseAmount)] | /ubl:Order/cac:OrderLine/cac:LineItem/cac:AllowanceCharge[cbc:MultiplierFactorNumeric and not(cbc:BaseAmount)]">
        <assert id="PEPPOL-T01-R020"
                test="false()"
                flag="fatal">[PEPPOL-T01-R020]-Allowance/charge base amount SHALL be provided when allowance/charge percentage is provided.</assert>
    </rule>
    ...
    <rule context="/ubl:Order/cac:AllowanceCharge[not(cbc:MultiplierFactorNumeric) and cbc:BaseAmount] | /ubl:Order/cac:OrderLine/cac:LineItem/cac:AllowanceCharge[not(cbc:MultiplierFactorNumeric) and cbc:BaseAmount]">
        <assert id="PEPPOL-T01-R021"
                test="false()"
                flag="fatal">[PEPPOL-T01-R021]-Allowance/charge percentage SHALL be provided when allowance/charge base amount is provided.</assert>
    </rule>
    ...
    <rule context="/ubl:Order/cac:AllowanceCharge |/ubl:Order/cac:OrderLine/cac:LineItem/cac:AllowanceCharge">
        <assert id="PEPPOL-T01-R022"
                ...
                flag="fatal">[PEPPOL-T01-R022]-Allowance/charge amount SHALL equal base amount * percentage/100 if base amount and percentage exists</assert>
        <assert id="PEPPOL-T01-R023"
                test="exists(cbc:AllowanceChargeReason) or exists(cbc:AllowanceChargeReasonCode)"
                flag="fatal">[PEPPOL-T01-R023]-Each document or line level allowance SHALL have an allowance reason text or an allowance reason code.</assert>
            <assert  id="PEPPOL-T01-R032"
                    test="number(cbc:Amount) &gt;= 0"
                    flag="fatal">[PEPPOL-T01-R032]-Allowance or charge amounts SHALL NOT be negative.</assert>
    </rule>
    ...
    <assert id="PEPPOL-T01-R029"
            ...
            flag="fatal">[PEPPOL-T01-R029]-Each Tax Category SHALL have a TAX category rate, except if the order is not subject to TAX.</assert>
    <assert id="PEPPOL-T01-R030"
            test="not(normalize-space(cbc:ID)='S') or (cbc:Percent) &gt; 0"
            flag="fatal">[PEPPOL-T01-R030]-When TAX category code is "Standard rated" (S) the TAX rate SHALL be greater than zero.</assert>
    </rule>
    ...
    <rule context="cac:OrderLine/cac:LineItem">
        ...
        <assert id="PEPPOL-T01-R024"
                ...
                flag="fatal">[PEPPOL-T01-R024]-Order line net amount SHALL equal (Ordered quantity * (Item net price/item price base quantity) + Order line charge amount - Order line allowance amount</assert>
        <assert id="PEPPOL-T01-R025"
                test="not(cac:Price/cbc:BaseQuantity) or xs:decimal(cac:Price/cbc:BaseQuantity) &gt; 0"
                flag="fatal">[PEPPOL-T01-R025]-Base quantity SHALL be a positive number above zero.</assert>
        <assert id="PEPPOL-T01-R001"
                test="count(key('k_lineId',cbc:ID)) = 1"
                flag="fatal">[PEPPOL-T01-R001]-Each order line SHALL have a document line identifier that is unique within the order.</assert>
        <assert id="PEPPOL-T01-R004"
                test="number(cbc:Quantity) &gt;=0"
                flag="fatal">[PEPPOL-T01-R004]-Each order line ordered quantity SHALL not be negative</assert>
        <assert id="PEPPOL-T01-R013"
                test="cbc:Quantity"
                flag="warning">[PEPPOL-T01-R013]-Each order line SHOULD have an ordered quantity</assert>
    </rule>
    ...
    <rule context="cac:Price/cac:AllowanceCharge">
        <assert id="PEPPOL-T01-R019"
                ...
                flag="fatal">[PEPPOL-T01-R019]-Item net price SHALL equal (Gross price - Allowance amount) when gross price is provided.</assert>
    </rule>
    ...
    <rule context="cac:Price">
        <assert id="PEPPOL-T01-R005"
                ...
                flag="fatal">[PEPPOL-T01-R005]-Each order line item net price SHALL not be negative
        </assert>
        <assert  id="PEPPOL-T01-R027"
                test="(cac:AllowanceCharge/cbc:BaseAmount) &gt;= 0 or not(exists(cac:AllowanceCharge/cbc:BaseAmount))"
                flag="fatal">[PEPPOL-T01-R027]-The Item gross price SHALL NOT be negative.</assert>
            <assert  id="PEPPOL-T01-R033"
                    test="number(cac:AllowanceCharge/cbc:Amount) &gt;= 0 or not(exists(cac:AllowanceCharge/cbc:Amount))"
                    flag="fatal">[PEPPOL-T01-R033]-Allowance or charge amounts SHALL NOT be negative.</assert>
    </rule>
        
    <!-- Quotation line reference -->
    <rule context="cac:QuotationLineReference/cbc:LineID">
        <assert id="PEPPOL-T01-R035"
                test="exists(/*/cac:QuotationDocumentReference/cbc:ID)"
                flag="fatal">[PEPPOL-T01-R035]-The quotation document reference MUST be provided when referencing a quotation line</assert>
    </rule>
        ...
        <rule context="cac:AllowanceCharge[cbc:ChargeIndicator = 'false']/cbc:AllowanceChargeReasonCode">
                <assert id="PEPPOL-T01-CL001"
                        ...
                        flag="fatal">[PEPPOL-T01-CL001]-Reason code MUST be according to subset of UNCL 5189 D.16B.</assert>
        </rule>
        ...
        <rule context="cac:AllowanceCharge[cbc:ChargeIndicator = 'true']/cbc:AllowanceChargeReasonCode">
                <assert id="PEPPOL-T01-CL002"
                        ...
                        satisfies normalize-space(text()) = $code"
                        flag="fatal">[PEPPOL-T01-CL002]-Reason code MUST be according to UNCL 7161 D.16B.</assert>
        </rule>
