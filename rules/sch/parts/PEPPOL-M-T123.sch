...
<pattern xmlns="http://purl.oclc.org/dsdl/schematron">
	...
	<rule context="cac:Period">
		...
		<assert id="PEPPOL-T123-R016" test= "not(cbc:StartTime) or  contains(cbc:StartTime, '+') or contains(cbc:StartTime, '-') or contains(cbc:StartTime, 'z') or contains(cbc:StartTime, 'Z')" flag="fatal">[PEPPOL-T123-R016] StartTime cannot be specified without timezone</assert>
		<assert id="PEPPOL-T123-R017" test= "not(cbc:EndTime) or contains(cbc:EndTime, '+') or contains(cbc:EndTime, '-') or contains(cbc:EndTime, 'z') or contains(cbc:EndTime, 'Z')" flag="fatal">[PEPPOL-T123-R017] EndTime cannot be specified without time zone</assert>
		<assert id="PEPPOL-T123-R019" test="not(cbc:StartTime) or count(timezone-from-time(cbc:StartTime)) &gt; 0" flag="fatal"> [PEPPOL-T123-R019] IssueTime MUST include timezone information.</assert>
		<assert id="PEPPOL-T123-R020" test="not(cbc:EndTime) or count(timezone-from-time(cbc:EndTime)) &gt; 0" flag="fatal"> [PEPPOL-T123-R020] IssueTime MUST include timezone information.</assert>
	</rule>
