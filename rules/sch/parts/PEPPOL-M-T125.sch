...
<pattern xmlns="http://purl.oclc.org/dsdl/schematron">
	...
	<rule context="cac:EstimatedDeliveryPeriod">
		<assert id="PEPPOL-T125-R011" test="cbc:EndDate or cbc:StartDate" flag="fatal">Start date or end date must be spefied in a period</assert>
		...
		<assert id="PEPPOL-T125-R013" test="not(cbc:EndTime) or ((cbc:EndTime) and (cbc:StartTime)) or ((cbc:EndDate) and (cbc:EndTime) and not(cbc:StartDate) and not(cbc:StartTime))" flag="fatal">EndTime cannot be specified without StartTime</assert>
	...
	<rule context="cac:EstimatedDespatchPeriod">
		...
		<assert id="PEPPOL-T125-R023" test="not(cbc:EndTime) or ((cbc:EndTime) and (cbc:StartTime)) or ((cbc:EndDate) and (cbc:EndTime) and not(cbc:StartDate) and not(cbc:StartTime))" flag="fatal">EndTime cannot be specified without StartTime</assert>
