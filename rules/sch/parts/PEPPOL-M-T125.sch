...
<pattern xmlns="http://purl.oclc.org/dsdl/schematron">
	...
	<rule context="cac:Shipment">
		<assert id="PEPPOL-T125-R003" test="not(cac:Consignment/cbc:TotalTransportHandlingUnitQuantity) or number(cac:Consignment/cbc:TotalTransportHandlingUnitQuantity) &gt;= 0" flag="warning">[PEPPOL-T125-R003] Total transport handling unit quantity SHALL not be negative</assert>
		<assert id="PEPPOL-T125-R004" test="not(cac:Consignment/cbc:TotalTransportHandlingUnitQuantity) or not(cac:Consignment/cac:TransportHandlingUnit) or number(cac:Consignment/cbc:TotalTransportHandlingUnitQuantity) = count(cac:Consignment/cac:TransportHandlingUnit)" flag="warning">[PEPPOL-T125-R004] Shipment transport handling unit quantity SHALL match the number of the transport handling units specified</assert>
		<assert id="PEPPOL-T125-R005" test="not(cac:Consignment/cbc:GrossWeightMeasure) or not(cac:Consignment/cac:TransportHandlingUnit/cac:MeasurementDimension[normalize-space(cbc:AttributeID) = 'AAB']/cbc:Measure) or (cac:Consignment/cbc:GrossWeightMeasure/@unitCode) &gt; (cac:Consignment/cac:TransportHandlingUnit/cac:MeasurementDimension[normalize-space(cbc:AttributeID) = 'AAB']/cbc:Measure/@unitCode) or (cac:Consignment/cbc:GrossWeightMeasure/@unitCode) &lt; (cac:Consignment/cac:TransportHandlingUnit/cac:MeasurementDimension[normalize-space(cbc:AttributeID) = 'AAB']/cbc:Measure/@unitCode) or number(cac:Consignment/cbc:GrossWeightMeasure) = sum(cac:Consignment/cac:TransportHandlingUnit/cac:MeasurementDimension[normalize-space(cbc:AttributeID) = 'AAB']/cbc:Measure)" flag="warning">[PEPPOL-T125-R005] Shipment  gross weight measure SHALL match the gross weight of the transport handling units specified</assert>
		<let name="THUGrossVolume" value="round(sum(cac:Consignment/cac:TransportHandlingUnit/cac:MeasurementDimension[normalize-space(cbc:AttributeID) = 'AAW']/cbc:Measure) * 1000)"/>
		<assert id="PEPPOL-T125-R006"
		   test="not(cac:Consignment/cbc:GrossVolumeMeasure) or (cac:Consignment/cbc:GrossVolumeMeasure/@unitCode) &gt; (cac:Consignment/cac:TransportHandlingUnit/cac:MeasurementDimension[normalize-space(cbc:AttributeID) = 'AAW']/cbc:Measure/@unitCode) or (cac:Consignment/cbc:GrossVolumeMeasure/@unitCode) &lt; (cac:Consignment/cac:TransportHandlingUnit/cac:MeasurementDimension[normalize-space(cbc:AttributeID) = 'AAW']/cbc:Measure/@unitCode) or ((cac:Consignment/cbc:GrossVolumeMeasure)/xs:decimal(.) * 1000) &gt;= $THUGrossVolume" flag="warning">
		...
		<assert id="PEPPOL-T125-R008" test="(cac:Consignment/cbc:GrossWeightMeasure) or cac:Consignment/(cbc:GrossVolumeMeasure) or (cac:Consignment/cbc:LoadingLengthMeasure)" flag="warning">[PEPPOL-T125-R008] Either gross weight, gross volume or loading length must be specified</assert>
		<assert id="PEPPOL-T125-R010" test ="not(cac:Consignment/cac:PaymentTerms) or cac:Consignment/cac:PaymentTerms/cbc:ID or cac:Consignment/cac:PaymentTerms/cbc:Note"	flag="warning">[PEPPOL-T125-R010] When Payment terms is specified, either the ID or the note must be specified</assert>			
	</rule>
