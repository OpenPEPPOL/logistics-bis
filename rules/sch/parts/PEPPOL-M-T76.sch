...
<pattern xmlns="http://purl.oclc.org/dsdl/schematron">
	...
	<rule context="cbc:CustomizationID">
			<assert id="PEPPOL-T76-R006" 
					...
					flag="fatal">[PEPPOL-T76-R006]-Specification identifier SHALL start with the value 'urn:fdc:peppol.eu:poacc:trns:order_response:3'.</assert>
	</rule>
    ...
    <rule context="cbc:PriceAmount">
        <assert id="PEPPOL-T76-R005"
            ...
            flag="fatal">[PEPPOL-T76-R005]-An order response SHALL be stated in a single currency</assert>
    ...
    <rule context="cac:BuyerCustomerParty/cac:Party">
        <assert id="PEPPOL-T76-R001"
            test="cac:PartyLegalEntity/cbc:RegistrationName or cac:PartyIdentification/cbc:ID"
                flag="fatal">[PEPPOL-T76-R001]-An order response SHALL have the buyer party official name or a buyer party identifier</assert>
    </rule>
    ...
    <rule context="cac:SellerSupplierParty/cac:Party">
        <assert id="PEPPOL-T76-R002"
                test="cac:PartyLegalEntity/cbc:RegistrationName or cac:PartyIdentification/cbc:ID"
                flag="fatal">[PEPPOL-T76-R002]-An order response SHALL have the seller party official name or a seller party identifier</assert>
    </rule>
    ...
    <rule context="cac:PromisedDeliveryPeriod | cac:OrderLine/cac:LineItem/cac:PromisedDeliveryPeriod">
        <assert  id="PEPPOL-T76-R004"
                ...
                flag="fatal">[PEPPOL-T76-R004]-If both delivery period start date and delivery period end date are given then the end date SHALL be later or equal to the start date.</assert>
   </rule>
    ...
    <rule context="cac:OrderLine/cac:LineItem">
        <assert id="PEPPOL-T76-R003"
                ...
                flag="fatal">[PEPPOL-T76-R003]-Each order response line SHALL have a document line identifier that is unique within the order.</assert>
    </rule>
	...
	<rule context="cbc:OrderResponseCode">
        ...
        flag="fatal">[PEPPOL-T76-R007]-An order response with code CA (Conditionally accepted) must provide order lines.</assert>
        <assert id="PEPPOL-T76-R008"
        test="(normalize-space(.) = 'AP' and count(../cac:OrderLine) = 0) or normalize-space(.) != 'AP'"
        flag="fatal">[PEPPOL-T76-R008]-An order response with code AP (Accepted) must NOT provide order lines.</assert>
        <assert id="PEPPOL-T76-R009"
        test="(normalize-space(.) = 'RE' and count(../cac:OrderLine) = 0) or normalize-space(.) != 'RE'"
        flag="fatal">[PEPPOL-T76-R009]-An order response with code RE (Rejected) must NOT provide order lines.</assert>
        <assert id="PEPPOL-T76-R010"
        test="(normalize-space(.) = 'AB' and count(../cac:OrderLine) = 0) or normalize-space(.) != 'AB'"
        flag="fatal">[PEPPOL-T76-R010]-An order response with code AB (Acknowledged) must NOT provide order lines.</assert>
	</rule>
