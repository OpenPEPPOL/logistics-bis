...
<pattern xmlns="http://purl.oclc.org/dsdl/schematron">
    ...
    <let name="TAXamount" value="if(/ubl:OrderChange/cac:TaxTotal/cbc:TaxAmount) then xs:decimal(/ubl:OrderChange/cac:TaxTotal/cbc:TaxAmount) else 0"/>
		...
		<rule context="cbc:CustomizationID">
				<assert id="PEPPOL-T114-R034"
						...
						flag="fatal">[PEPPOL-T114-R034]-Specification identifier MUST start with the value 'urn:fdc:peppol.eu:poacc:trns:order_change:3'.</assert>
		</rule>
    ...
    <rule context="cbc:Amount | cbc:TaxAmount | cbc:LineExtensionAmount | cbc:PriceAmount | cbc:BaseAmount | cac:AnticipatedMonetaryTotal/cbc:*">
        <assert id="PEPPOL-T114-R003"
                ...
                flag="fatal">[PEPPOL-T114-R003]-An order MUST be stated in a single currency</assert>
        <assert id="PEPPOL-T114-R028"
                test="ancestor::node()/local-name() = 'Price' or string-length(substring-after(., '.')) &lt;= 2"
                flag="fatal">[PEPPOL-T114-R028]-Elements of data type amount cannot have more than 2 decimals (I.e. all amounts except unit price amounts)</assert>
    </rule>
    ...
    <rule context="ubl:OrderChange">
        <assert id="PEPPOL-T114-R002"
                ...
                flag="warning">[PEPPOL-T114-R002]-An order change SHOULD provide information about its validity end date.</assert>
    </rule>
    ...
    <rule context="cac:OriginatorCustomerParty">
        <assert id="PEPPOL-T114-R014"
                ...
                flag="fatal">[PEPPOL-T114-R014]-An order change MUST have the originator party name or an identifier</assert>
    </rule>
    ...
    <rule context="cac:PartyTaxScheme[cac:TaxScheme/cbc:ID='VAT']">
        <assert id="PEPPOL-T114-R026"
                ...
                flag="fatal">[PEPPOL-T114-R026]-When TAX is VAT then Party VAT identifiers MUST have a prefix in accordance with ISO code ISO 3166-1 alpha-2 by which the country of issue may be identified. Nevertheless, Greece may use the prefix ‘EL’.</assert>
    </rule>
    ...
    <rule context="cac:AnticipatedMonetaryTotal">
        ...
        <assert id="PEPPOL-T114-R006"
                ...
                flag="fatal">[PEPPOL-T114-R006]-Expected total amount for payment MUST NOT be negative</assert>
        <assert id="PEPPOL-T114-R007"
                test="$lineEtensionAmount &gt;=0"
                flag="fatal">[PEPPOL-T114-R007]-Expected total sum of line amounts MUST NOT be negative</assert>
        <assert id="PEPPOL-T114-R008"
                test="$lineEtensionAmount = $sumLineExtensionAmount"
                flag="warning">[PEPPOL-T114-R008]-Expected total sum of line amounts MUST equal the sum of the order line amounts at order line level</assert>
        <assert id="PEPPOL-T114-R009"
                test="$allowanceTotalAmount = $sumAllowance"
                flag="fatal">[PEPPOL-T114-R009]-Expected total sum of allowance at document level MUST be equal to the sum of allowance amounts at document level</assert>
        <assert id="PEPPOL-T114-R010"
                test="$chargeTotalAmount= $sumCharge"
                flag="fatal">[PEPPOL-T114-R010]-Expected total sum of charges at document level MUST be equal to the sum of charge amounts at document level</assert>

        <assert id="PEPPOL-T114-R011"
                test="$taxexclusiveAmount = $lineEtensionAmount - $allowanceTotalAmount + $chargeTotalAmount"
                flag="fatal">[PEPPOL-T114-R011]-Expected total amount without TAX = Expected total sum of line amounts - Sum of allowances on document level + Sum of charges on document level</assert>
        <assert id="PEPPOL-T114-R016"
                ...
                flag="fatal">[PEPPOL-T114-R016]-Amount due for payment = Order change total amount with TAX - Paid amount + Rounding amount.</assert>
        <assert id="PEPPOL-T114-R017"
                ...
                flag="fatal">[PEPPOL-T114-R017]-Expected total amount with TAX = Expected total amount without TAX + Order total TAX amount.</assert>
    </rule>
    ...
    <rule context="cac:AllowanceCharge[cbc:MultiplierFactorNumeric and not(cbc:BaseAmount)] | /ubl:OrderChange/cac:OrderLine/cac:LineItem/cac:AllowanceCharge[cbc:MultiplierFactorNumeric and not(cbc:BaseAmount)]">
        <assert id="PEPPOL-T114-R020"
                test="false()"
                flag="fatal">[PEPPOL-T114-R020]-Allowance/charge base amount MUST be provided when allowance/charge percentage is provided.</assert>
    </rule>
    ...
    <rule context="/ubl:OrderChange/cac:AllowanceCharge[not(cbc:MultiplierFactorNumeric) and cbc:BaseAmount] | /ubl:OrderChange/cac:OrderLine/cac:LineItem/cac:AllowanceCharge[not(cbc:MultiplierFactorNumeric) and cbc:BaseAmount]">
        <assert id="PEPPOL-T114-R021"
                test="false()"
                flag="fatal">[PEPPOL-T114-R021]-Allowance/charge percentage MUST be provided when allowance/charge base amount is provided.</assert>
    </rule>
    ...
    <rule context="/ubl:OrderChange/cac:AllowanceCharge |/ubl:OrderChange/cac:OrderLine/cac:LineItem/cac:AllowanceCharge">
        <assert id="PEPPOL-T114-R022"
                ...
                flag="fatal">[PEPPOL-T114-R022]-Allowance/charge amount MUST equal base amount * percentage/100 if base amount and percentage exists</assert>
        <assert id="PEPPOL-T114-R023"
                test="exists(cbc:AllowanceChargeReason) or exists(cbc:AllowanceChargeReasonCode)"
                flag="fatal">[PEPPOL-T114-R023]-Each document or line level allowance MUST have an allowance reason text or an allowance reason code.</assert>
            <assert  id="PEPPOL-T114-R032"
                    test="number(cbc:Amount) &gt;= 0"
                    flag="fatal">[PEPPOL-T114-R032]-Allowance or charge amounts MUST NOT be negative.</assert>
    </rule>
    ...
    <assert id="PEPPOL-T114-R029"
            ...
            flag="fatal">[PEPPOL-T114-R029]-Each Tax Category MUST have a TAX category rate, except if the order is not subject to TAX.</assert>
    <assert id="PEPPOL-T114-R030"
            test="not(normalize-space(cbc:ID)='S') or (cbc:Percent) &gt; 0"
            flag="fatal">[PEPPOL-T114-R030]-When TAX category code is "Standard rated" (S) the TAX rate MUST be greater than zero.</assert>
    </rule>
    ...
    <rule context="cac:OrderLine/cac:LineItem">
        ...
        <assert id="PEPPOL-T114-R024"
                ...
                flag="fatal">[PEPPOL-T114-R024]-Order line net amount MUST equal (Ordered quantity * (Item net price/item price base quantity) + Order line charge amount - Order line allowance amount</assert>
        <assert id="PEPPOL-T114-R025"
                test="not(cac:Price/cbc:BaseQuantity) or xs:decimal(cac:Price/cbc:BaseQuantity) &gt; 0"
                flag="fatal">[PEPPOL-T114-R025]-Base quantity MUST be a positive number above zero.</assert>
        <assert id="PEPPOL-T114-R001"
                test="count(key('k_lineId',cbc:ID)) = 1"
                flag="fatal">[PEPPOL-T114-R001]-Each order line MUST have a document line identifier that is unique within the order.</assert>
        <assert id="PEPPOL-T114-R004"
                test="number(cbc:Quantity) &gt;=0"
                flag="fatal">[PEPPOL-T114-R004]-Each order line ordered quantity MUST not be negative</assert>
        <assert id="PEPPOL-T114-R013"
                test="cbc:Quantity"
                flag="warning">[PEPPOL-T114-R013]-Each order line SHOULD have an ordered quantity</assert>
    </rule>
    ...
    <rule context="cac:Price/cac:AllowanceCharge">
        <assert id="PEPPOL-T114-R019"
                ...
                flag="fatal">[PEPPOL-T114-R019]-Item net price MUST equal (Gross price - Allowance amount) when gross price is provided.</assert>
    </rule>
    ...
    <rule context="cac:Price">
        <assert id="PEPPOL-T114-R005"
                ...
                flag="fatal">[PEPPOL-T114-R005]-Each order line item net price MUST not be negative
        </assert>
        <assert  id="PEPPOL-T114-R027"
                test="(cac:AllowanceCharge/cbc:BaseAmount) &gt;= 0 or not(exists(cac:AllowanceCharge/cbc:BaseAmount))"
                flag="fatal">[PEPPOL-T114-R027]-The Item gross price MUST NOT be negative.</assert>
            <assert  id="PEPPOL-T114-R033"
                    test="number(cac:AllowanceCharge/cbc:Amount) &gt;= 0 or not(exists(cac:AllowanceCharge/cbc:Amount))"
                    flag="fatal">[PEPPOL-T114-R033]-Allowance or charge amounts MUST NOT be negative.</assert>
    </rule>
    ...
    <rule context="cac:QuotationLineReference/cbc:LineID">
        <assert id="PEPPOL-T114-R035"
                ...
                flag="fatal">[PEPPOL-T114-R035]-The quotation document reference MUST be provided when referencing a quotation line</assert>
    </rule>
    ...
    <rule context="cac:AllowanceCharge[cbc:ChargeIndicator = 'false']/cbc:AllowanceChargeReasonCode">
                <assert id="PEPPOL-T114-CL001"
                        ...
                        flag="fatal">[PEPPOL-T114-CL001]-Reason code MUST be according to subset of UNCL 5189 D.16B.</assert>
        </rule>
        ...
        <rule context="cac:AllowanceCharge[cbc:ChargeIndicator = 'true']/cbc:AllowanceChargeReasonCode">
                <assert id="PEPPOL-T114-CL002"
                        ...
                        satisfies normalize-space(text()) = $code"
                        flag="fatal">[PEPPOL-T114-CL002]-Reason code MUST be according to UNCL 7161 D.16B.</assert>
        </rule> 
</pattern>
