...
<pattern xmlns="http://purl.oclc.org/dsdl/schematron">
	...
	<rule context="cbc:CustomizationID">
			<assert id="PEPPOL-T110-R030" 
					...
					flag="fatal">[PEPPOL-T110-R030]-Specification identifier SHALL start with the value 'urn:fdc:peppol.eu:poacc:trns:order_agreement:3'.</assert>
	</rule>
	...
	<rule context="cac:Item">
		<assert id= "PEPPOL-T110-R002"
				...
				flag="fatal">[PEPPOL-T110-R002]-Each item in an Order agreement line SHALL be identifiable by either "item sellers identifier" or "item standard identifier"</assert>
	</rule>
	...
	<rule context="cbc:Amount | cbc:TaxAmount | cbc:TaxableAmount | cbc:LineExtensionAmount | cbc:PriceAmount | cbc:BaseAmount | cac:LegalMonetaryTotal/cbc:*">
		<assert id="PEPPOL-T110-R004"
				...
				flag="fatal">[PEPPOL-T110-R004]-All amounts SHALL have same currency code as document currency</assert>
		<assert id="PEPPOL-T110-R013"
				test="ancestor::node()/local-name() = 'Price' or string-length(substring-after(., '.')) &lt;= 2"
				flag="fatal">[PEPPOL-T110-R013]-Elements of data type amount cannot have more than 2 decimals (I.e. all amounts except unit price amounts)</assert>
	</rule>
...
<!-- TAX rules -->
	...
	<rule context="cac:TaxTotal/cac:TaxSubtotal">
		...
		<assert id="PEPPOL-T110-R024"
			...
			flag="fatal">[PEPPOL-T110-R024]-TAX category tax amount = TAX category taxable amount  x (TAX category rate  / 100), rounded to two decimals.</assert>
	...
	<rule context="/ubl:OrderResponse/cac:TaxTotal[cac:TaxSubtotal]">
		<assert id="PEPPOL-T110-R025"
			...
			flag="fatal">[PEPPOL-T110-R025]-If TAX breakdown is present, the order agreement TAX total amount  = Σ TAX category tax amount.</assert>
	</rule>
	...
	<rule context="cac:TaxSubtotal/cac:TaxCategory[not(cbc:TaxExemptionReason)]">
		<assert id="PEPPOL-T110-R028"
			...
			flag="fatal">[PEPPOL-T110-R028]-A TAX breakdown with TAX Category codes E, AE, K, G or O SHALL have a TAX exemption reason text </assert>
	</rule>
	...
	<rule context="cac:TaxSubtotal/cac:TaxCategory[cbc:TaxExemptionReason]">
		<assert id="PEPPOL-T110-R029"
			...
			flag="fatal">[PEPPOL-T110-R029]-A TAX breakdown with TAX Category codes S, Z, L and M SHALL NOT have a TAX exemption reason text </assert>
	</rule>
	...
	<rule context="cac:AllowanceCharge/cac:TaxCategory | cac:Item/cac:ClassifiedTaxCategory">
		...
		<assert id="PEPPOL-T110-R026"
			...
			flag="fatal">[PEPPOL-T110-R026]-Tax category rates MUST match provided tax categories on document level when such exists.</assert>
    ...
    <!-- Note: Only  TaxTotal is present on header level -->
		<assert id="PEPPOL-T110-R027"
			...
			flag="fatal">[PEPPOL-T110-R027]-Tax categories MUST match provided tax categories on document level when such exists.</assert>
			

		<assert id="PEPPOL-T110-R019"
			test="cbc:Percent or (normalize-space(cbc:ID)='O')"
			flag="fatal">[PEPPOL-T110-R019]-Each Tax Category SHALL have a TAX category rate, except if the order is not subject to TAX.</assert>
		<assert id="PEPPOL-T110-R020"
			test="not(normalize-space(cbc:ID)='S') or (cbc:Percent) &gt; 0"
			flag="fatal">[PEPPOL-T110-R020]-When TAX category code is "Standard rated" (S) the TAX rate SHALL be greater than zero.</assert>
...
<!-- Document totals -->
	...
	<rule context="cac:LegalMonetaryTotal">
		...
		<assert id="PEPPOL-T110-R014"
				...
				flag="fatal">[PEPPOL-T110-R014]-If document totals is provided, all order agreement lines SHALL have a line extension amount</assert>

		<assert id="PEPPOL-T110-R005"
				test="not(cbc:PayableAmount) or cbc:PayableAmount &gt;= 0"
				flag="fatal">[PEPPOL-T110-R005]-Total amount for payment SHALL NOT be negative, if expected total amount for payment is provided.</assert>

		<assert id="PEPPOL-T110-R006"
				test="$lineExtensionAmount &gt;= 0"
				flag="fatal">[PEPPOL-T110-R006]-Total amount for payment SHALL NOT be negative, if expected total amount for payment is provided.</assert>

		<assert id="PEPPOL-T110-R007"
				test="not(cbc:LineExtensionAmount) or $lineExtensionAmount = $lineExtensionTotal"
				flag="fatal">[PEPPOL-T110-R007]-Total sum of line amounts SHALL equal the sum of the order line amounts at order line level, if total sum of line amounts is provided.</assert>

		<assert id="PEPPOL-T110-R008"
				test="not(cbc:ChargeTotalAmount) or $chargeTotalAmount = $chargeTotal"
				flag="fatal">[PEPPOL-T110-R008]-Total sum of charges at document level SHALL be equal to the sum of charges at document level, if total sum of charges at document level is provided.</assert>

		<assert id="PEPPOL-T110-R009"
				test="not(cbc:AllowanceTotalAmount) or $allowanceTotalAmount = $allowanceTotal"
				flag="fatal">[PEPPOL-T110-R009]-Total sum of allowance at document level SHALL be equal to the sum of allowance amounts at document level, if total sum of allowance at document level is provided.</assert>
		...
		<assert id="PEPPOL-T110-R010"
				...
				flag="fatal">[PEPPOL-T110-R010]-Tax exclusive amount SHALL equal the sum of line amount plus total charge amount at document level less total allowance amount at document level if tax exclusive amount is provided.</assert>

		<assert id="PEPPOL-T110-R011"
				test="$taxInclusiveAmount = $taxExclusiveAmount + $taxTotal"
				flag="fatal">[PEPPOL-T110-R011]-Tax inclusive amount SHALL equal tax exclusive amount plus total tax amount.</assert>
		...
		<assert id="PEPPOL-T110-R012"
				...
				flag="fatal">[PEPPOL-T110-R012]-Total amount for payment SHALL be equal to the tax inclusive amount minus the prepaid amount plus rounding amount</assert>
	...
	<rule context="/ubl:OrderResponse/cac:AllowanceCharge[cbc:MultiplierFactorNumeric and not(cbc:BaseAmount)]">
		<assert id="PEPPOL-T110-R015"
				test="false()"
				flag="fatal">[PEPPOL-T110-R015]-Allowance/charge base amount SHALL be provided when allowance/charge percentage is provided.</assert>
	</rule>
	...
	<rule context="/ubl:OrderResponse/cac:AllowanceCharge[not(cbc:MultiplierFactorNumeric) and cbc:BaseAmount]">
		<assert id="PEPPOL-T110-R016"
				test="false()"
				flag="fatal">[PEPPOL-T110-R016]-Allowance/charge percentage SHALL be provided when allowance/charge base amount is provided.</assert>
	</rule>
	...
	<rule context="/ubl:OrderResponse/cac:AllowanceCharge">
		<assert id="PEPPOL-T110-R017"
				...
				flag="fatal">[PEPPOL-T110-R017]-Allowance/charge amount SHALL equal base amount * percentage/100 if base amount and percentage exists</assert>
		<assert id="PEPPOL-T110-R018"
				test="exists(cbc:AllowanceChargeReason) or exists(cbc:AllowanceChargeReasonCode)"
				flag="fatal">[PEPPOL-T110-R018]-Each document level allowance SHALL have an allowance reason text or an allowance reason code.</assert>
		<assert  id="PEPPOL-T110-R021"
			test="number(cbc:Amount) &gt;= 0"
			flag="fatal">[PEPPOL-T110-R021]-Document level allowance or charge amounts SHALL NOT be negative.</assert>
	</rule>
	...
	<rule context="cac:Price">
		<assert id="PEPPOL-T110-R001"
			...
			flag="fatal">[PEPPOL-T110-R001]-Each order agreement line item net price SHALL not be negative
		</assert>
		<assert  id="PEPPOL-T110-R022"
			test="(cac:AllowanceCharge/cbc:BaseAmount) &gt;= 0 or not(exists(cac:AllowanceCharge/cbc:BaseAmount))"
			flag="fatal">[PEPPOL-T110-R022]-The Item gross price SHALL NOT be negative.</assert>
		<assert  id="PEPPOL-T110-R023"
			test="number(cac:AllowanceCharge/cbc:Amount) &gt;= 0 or not(exists(cac:AllowanceCharge/cbc:Amount))"
			flag="fatal">[PEPPOL-T110-R023]-Allowance or charge price amounts SHALL NOT be negative.</assert>
	</rule>
	...
	<rule context="cac:AllowanceCharge[cbc:ChargeIndicator = 'false']/cbc:AllowanceChargeReasonCode">
		<assert id="PEPPOL-T110-CL001"
			...
			flag="fatal">[PEPPOL-T110-CL001]-Reason code MUST be according to subset of UNCL 5189 D.16B.</assert>
	</rule>
	...
	<rule context="cac:AllowanceCharge[cbc:ChargeIndicator = 'true']/cbc:AllowanceChargeReasonCode">
		<assert id="PEPPOL-T110-CL002"
			...
			satisfies normalize-space(text()) = $code"
			flag="fatal">[PEPPOL-T110-CL002]-Reason code MUST be according to UNCL 7161 D.16B.</assert>
	</rule>
</pattern>