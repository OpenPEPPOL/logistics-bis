...
<pattern xmlns="http://purl.oclc.org/dsdl/schematron">
	...
	<rule context="cac:Shipment/cac:Consignment/cac:Status">
		<assert id="PEPPOL-T128-R025" test="((normalize-space(cbc:ConditionCode) &gt;= 'AQ') and (cbc:StatusReasonCode)) " flag="fatal">If the Consignment is Conditiobally Accepted (CA/RE), a status reason code SHALL be provided. </assert>
		<assert id="PEPPOL-T128-R026" test="((normalize-space(cbc:ConditionCode) &gt;= 'AQ') and (cbc:StatusReason)) " flag="fatal">If the Consignment is Conditiobally Accepted (CA/RE), a status reason (text) SHALL be provided. </assert>
	</rule>
</pattern>
